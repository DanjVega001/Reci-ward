import 'package:flutter/material.dart';

class Pallete {
  static const Color color1 = Color(0xFF53B175);
//  static const Color color1 = Color.fromRGBO(50, 118, 87, 1);
  static const Color color2 = Color.fromARGB(255, 222, 231, 226);

  static const Color color3 = Color(0xFF5EC465);
  static const Color color4 = Color(0xFF9ED48F);
  static const Color colorGradient = Color(0xFFD8FCD8);
  static const Color colorWhite = Colors.white;
  static const Color colorGrey = Color.fromARGB(255, 242, 242, 242);
  static const Color colorGrey2 = Color.fromARGB(255, 249, 249, 249);
  static const Color colorGrey3 = Color.fromARGB(255, 77, 76, 76);
  static const Color colorBlack = Color.fromARGB(255, 0, 0, 0);

}
